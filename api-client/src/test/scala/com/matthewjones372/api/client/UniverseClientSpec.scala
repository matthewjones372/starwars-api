...
object UniverseClientSpec extends ZIOSpecDefault:
  def spec = suite("UniverseClientLive Spec")(
    suite("API Response Behavior")(
      test("can resolve the correct films from a person") {
        for
          ...
          f1  <- UniverseClient.getFilmsFromCharacter(1).fork
      ...
      test("maps every person to the films they appear in") {
        for
          ...
          res <- UniverseClient.getFilmsFromCharacters
        yield assertTrue(
          res.size == 12,
      ...
      test("can resolve all people from a paged response") {
        for
          _   <- TestClient.addRequestResponse(personPagedRequest, response = pagedPersonResponse)
          res <- UniverseClient.getCharacters
      ...
      test("returns the correct error when failing to get paged responses") {
        ...
        for
          ...
          f1  <- UniverseClient.getCharacters.exit.fork
          _   <- TestClock.adjust(5.seconds)
          res <- f1.join
    ...
    suite("API Error Behavior")(
      test("returns the correct error when an entity is not found") {
        ...
        for
          ...
          f1  <- UniverseClient.getFilmsFromCharacter(1).exit.fork
          _   <- TestClock.adjust(5.seconds)
          res <- f1.join
        yield {
      ...
      suite("Retrying Behavior")(
        test("API retries a request when there is a sever error") {
          def getPersonWithInitialFailure(state: Ref[Int]) = ApiServer.getCharacterEndpoint.implement { _ =>
        ...
        test("API does not retry a request when there is a clientError") {
          for
            ...
            request       <- UniverseClient.getFilmsFromCharacter(1).exit.fork
            _             <- TestClock.adjust(10.seconds)
            result        <- request.join
    ...
    suite("API Caching behavior")(
      test("calls the cache on the first call") {
        def successfulPersonCall(state: Ref[Boolean]) = ApiServer.getCharacterEndpoint.implement { _ =>
          state.modify {
        ...
        for
          ...
          r1      <- UniverseClient.getFilmsFromCharacter(1)
          r2      <- UniverseClient.getFilmsFromCharacter(1)
          _       <- TestClock.adjust(5.seconds)
