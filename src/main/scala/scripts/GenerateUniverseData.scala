...
object GenerateUniverseData extends ZIOAppDefault:
  ...
  private final case class Source(universe: UniverseId, series: String, scraped: Boolean)
  ...
  private val sources = List(
    Source(UniverseId.StarWars, "Q22092344", scraped = false),
    Source(UniverseId.Marvel, "Q642878", scraped = true),
    Source(UniverseId.MiddleEarth, "Q190214", scraped = true),
    Source(UniverseId.Wizarding, "Q216930", scraped = true)
  )
  ...
  private def filmsQuery(series: String) =
  ...
  private def castQuery(series: String) =
  ...
  private def charactersQuery(series: String) =
    s"""SELECT ?character (SAMPLE(?label) AS ?name)
       ...
       |  ?film wdt:P179 wd:$series ; p:P161/pq:P453 ?character .
  ...
  private def fetch(source: Source): RIO[Client & Scope, Raw] =
    for
      ...
      films      <- ask(filmsQuery(source.series))
      cast       <- ask(castQuery(source.series))
      characters <- if source.scraped then ask(charactersQuery(source.series)) else ZIO.succeed(Nil)
      actors     <- ask(actorsQuery(source.series))
      _          <- ZIO.logInfo(
  ...
  private def run(raws: List[Raw]): Task[Unit] =
    ...
    for
      ...
      byActor    = assembled.flatMap(_._3._1).groupBy(_._1)
      ...
      creditsBy = assembled.flatMap(_._3._2).groupMap(_._1)(_._2)
      ...
      actors    = actorIds.toList.sortBy(_._2).map { (q, _) =>
                 ...
                 val roles = byActor.getOrElse(q, Nil).map(_._2).sorted
                 Actor(
                   name = row.get("name").orElse(labelsBy.get(q)).getOrElse(q),
                   films = creditsBy.getOrElse(q, Nil).toSet,
                   universes = roles.map(_.universe).toSet,
                   roles = roles,
                   attributes = Map(
                     "wikidata"    -> q,
      ...
      _       <- ZIO.foreachDiscard(assembled)((universe, entities, _) => write(universe, entities))
  ...
  private def write(universe: UniverseId, entities: (List[Character], List[Film])) =
    val (characters, films) = entities
    // Star Wars keeps the swapi entities it already ships; only the cast links
    // are new, and they are matched onto it by name.
    if universe == UniverseId.StarWars then
      for
        existing <- readEntities(universe)
        byName    = characters.map(person => person.name.toLowerCase -> person).toMap
        merged    =
          existing._1.map { person =>
            byName.get(person.name.toLowerCase).fold(person)(scraped => person.copy(portrayedBy = scraped.portrayedBy))
          }
        castByFilm  = films.map(film => film.title.toLowerCase -> film.cast).toMap
        mergedFilms = existing._2.map(film => film.copy(cast = castByFilm.getOrElse(film.title.toLowerCase, Set.empty)))
        _          <- ZIO.logInfo(
               s"  matched ${merged.count(_.portrayedBy.nonEmpty)} of ${merged.size} star wars characters to actors"
             )
        _ <- writeFile(s"${universe.slug}_people.json", encodeAs(merged))
        _ <- writeFile(s"${universe.slug}_films.json", encodeAs(mergedFilms))
      yield ()
    else
      writeFile(s"${universe.slug}_people.json", encodeAs(characters)) *>
        writeFile(s"${universe.slug}_films.json", encodeAs(films))
